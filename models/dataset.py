...
class Dataset(object):
    def __init__(self, dataset, fold, iter, split, batch=128):
        self.dataset = dataset
        self.path = "./data/{}.sdf".format(dataset)  # "../../data/{}.sdf".format(dataset)
        ...
        if split == "":
            ...
            self.load_dataset_random(fold, iter)
        else:
    ...
    def load_dataset_stratified_sampling(self, fold, iter):
        ...
        if self.task != "regression":
            self.mols, self.coords, self.target = np.array(x), np.array(c), np.array(y, dtype=object)  #np.array(y, dtype=int)
        else:
            self.mols, self.coords, self.target = np.array(x), np.array(c), np.array(y)

        print('len!!!')
        print(self.mols.shape, self.coords.shape, self.target.shape)
